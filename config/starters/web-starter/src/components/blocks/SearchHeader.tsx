...
export function SearchHeader({
  ...
  return (
    <header className={`relative overflow-hidden ${compact ? '' : 'border-b border-border'}`}>
      {!compact && (
        <>
      ...
      <Container className={`relative text-center ${compact ? 'py-10 md:py-12' : 'py-20 md:py-28'}`}>
        <motion.h1
          initial={{ opacity: 0, y: 16 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.7, ease: [0.16, 1, 0.3, 1] }}
          className={`text-balance font-semibold ${compact ? 'text-3xl md:text-4xl' : 'text-4xl md:text-6xl'}`}
        >
        ...
        <motion.p
          ...
          className={`mx-auto text-muted ${compact ? 'mt-3 max-w-lg text-base' : 'mt-4 max-w-xl text-lg'}`}
        >
          {subtitle}
        </motion.p>
        {!compact && (
