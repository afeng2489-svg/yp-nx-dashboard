...
const MAPPING_TYPE_LABELS: Record<MappingType, { label: string; icon: React.ReactNode; color: string }> = {
  main: { label: '主模型', icon: <Sparkles className="w-3 h-3" />, color: 'bg-blue-500/10 text-blue-600 border-blue-500/20' },
  thinking: { label: '推理模型 (Thinking)', icon: <Brain className="w-3 h-3" />, color: 'bg-purple-500/10 text-purple-600 border-purple-500/20' },
  haiku: { label: 'Haiku 默认', icon: <Sparkles className="w-3 h-3" />, color: 'bg-green-500/10 text-green-600 border-green-500/20' },
  sonnet: { label: 'Sonnet 默认', icon: <Sparkles className="w-3 h-3" />, color: 'bg-orange-500/10 text-orange-600 border-orange-500/20' },
  opus: { label: 'Opus 默认', icon: <Sparkles className="w-3 h-3" />, color: 'bg-red-500/10 text-red-600 border-red-500/20' },
};
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        >
          {showAddForm ? '取消' : <><Plus className="w-3 h-3 inline mr-1" />添加映射</>}
        </button>
      </div>
        ...
        <div className="p-3 bg-muted/50 rounded-lg space-y-3">
          <div>
            ...
            <select
              ...
              onChange={(e) => setNewMapping({ ...newMapping, mapping_type: e.target.value as MappingType })}
          ...
          {(Object.keys(MAPPING_TYPE_LABELS) as MappingType[]).map((type) => {
            ...
            return (
              <div key={type} className="space-y-2">
                <div className={cn('inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs border', typeInfo.color)}>
                  {typeInfo.icon}
                  {typeInfo.label}
                </div>
