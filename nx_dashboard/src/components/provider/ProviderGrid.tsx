...
import { AIProvider, ProviderPreset, CreateProviderRequest, UpdateProviderRequest, ModelMapping, ConnectionTestResult, SelectedModelResponse } from '@/api/client';
...
interface ProviderGridProps {
  ...
  onAddMapping: (providerId: string, mapping: { mapping_type: string; model_id: string; display_name?: string }) => Promise<void>;
  ...
  onEnableProvider: (providerId: string, model?: string) => Promise<{ success: boolean; message: string; model: string } | null>;
  onDisableProvider: (providerId: string) => Promise<{ success: boolean; message: string; model: string } | null>;
...
export function ProviderGrid({
  ...
  const [connectionStatus, setConnectionStatus] = useState<Record<string, { testing: boolean; result?: ConnectionTestResult }>>({});
  ...
  const isProviderEnabled = selectedProvider && selectedModel
  ...
  const handleTestConnection = async (providerId: string) => {
    ...
    try {
      ...
      setConnectionStatus(prev => ({ ...prev, [providerId]: { testing: false, result: { success: false, message: String(error) } } }));
    }
  };
        ...
        <div className="mt-6 p-4 bg-card rounded-xl border border-border">
          ...
          {connectionStatus[selectedProvider.id]?.result && (
            <div className={cn(
              ...
              <p className="font-medium">{connectionStatus[selectedProvider.id].result!.message}</p>
              {connectionStatus[selectedProvider.id].result!.models && (
                <p className="mt-1 text-xs opacity-80">
                  可用模型: {connectionStatus[selectedProvider.id].result!.models?.slice(0, 5).join(', ')}
