...
import { useAgentExecutionV2 } from '@/hooks/useAgentExecutionV2';
...
export function ConversationViewV2({ teamId, onClose }: ConversationViewProps) {
  ...
  const agentExec = useAgentExecutionV2();
  const isActive =
    agentExec.status === 'started' ||
    agentExec.status === 'thinking' ||
    agentExec.status === 'confirmation';
  const isStreaming = isActive && agentExec.partialOutput.length > 0;
        ...
        {activeTab === 'chat' && (
          <>
            ...
            <div ref={messagesScrollRef} className="flex-1 overflow-y-auto p-4 space-y-4">
              ...
              {/* 流式 AI 回复气泡 — 执行中实时渲染 partialOutput */}
              {isStreaming && (
                <div className="flex gap-3 justify-start">
                  ...
                  <div className="max-w-[80%] rounded-2xl px-4 py-2.5 bg-muted">
                    <MarkdownMessage content={agentExec.partialOutput} variant="assistant" />
                    <span className="inline-block w-2 h-4 ml-0.5 bg-emerald-400 animate-pulse rounded-sm" />
                  </div>
                </div>
              )}
              {/* 等待中（thinking）无输出时显示加载指示 */}
