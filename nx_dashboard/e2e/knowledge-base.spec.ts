...
test.describe('Knowledge Base', () => {
  ...
  test('upload document', async () => {
    ...
    form.append(
      'file',
      new Blob(['NexusFlow is an AI software factory'], { type: 'text/plain' }),
      'test.txt',
    );
    const res = await fetch(`${API_BASE}/api/v1/knowledge-bases/upload`, {
      method: 'POST',
      body: form,
    });
    expect([200, 201]).toContain(res.status);
  ...
  test.afterAll(async () => {
    if (docId)
      await fetch(`${API_BASE}/api/v1/knowledge-bases/${docId}`, { method: 'DELETE' }).catch(
        () => {},
      );
    if (kbId)
      await fetch(`${API_BASE}/api/v1/knowledge-bases/${kbId}`, { method: 'DELETE' }).catch(
        () => {},
      );
  });
});
