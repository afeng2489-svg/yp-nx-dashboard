...
pub async fn get_interrupted_executions(
    ...
    let service = state.resume_service.as_ref()
        .ok_or_else(|| (StatusCode::SERVICE_UNAVAILABLE, Json(serde_json::json!({ "error": "Resume service not available" }))))?;
...
pub async fn resume_execution(
    ...
    // 查找 checkpoint
    let interrupted = service.find_interrupted().map_err(map_tev_error)?;
    let checkpoint = interrupted.into_iter()
    ...
    // 清理旧 checkpoint
    service.delete_checkpoint(&execution_id).map_err(map_tev_error)?;
...
pub async fn trigger_crash_detection(
    ...
    let detector = state.crash_detector.as_ref()
        .ok_or_else(|| (StatusCode::SERVICE_UNAVAILABLE, Json(serde_json::json!({ "error": "Crash detector not available" }))))?;
...
pub async fn trigger_temp_cleanup(
    State(state): State<Arc<AppState>>,
) -> Result<Json<crate::services::team_evolution::temp_cleaner::TempCleanResult>, (StatusCode, Json<serde_json::Value>)> {
    let cleaner = state.temp_cleaner.as_ref()
        .ok_or_else(|| (StatusCode::SERVICE_UNAVAILABLE, Json(serde_json::json!({ "error": "Temp cleaner not available" }))))?;
