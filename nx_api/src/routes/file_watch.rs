...
pub async fn get_file_changes(
    ...
    let watcher = state.file_watcher.as_ref().ok_or_else(|| {
        (
            StatusCode::SERVICE_UNAVAILABLE,
            Json(serde_json::json!({ "error": "File watcher not available" })),
        )
    })?;
    ...
    if !watcher.is_enabled() {
        return Err((
            ...
            Json(serde_json::json!({ "error": "File watcher is disabled" })),
        ));
    }
...
pub async fn start_file_watch(
    ...
    if workspace.is_empty() {
        return Err((
            ...
            Json(serde_json::json!({ "error": "No workspace path set" })),
        ));
    }
