...
use nx_memory::{
    EmbeddingProvider, MemoryChunk, MemorySearch, MemoryStore, SearchRequest, StoreRequest,
    Transcript,
};
...
pub async fn store_memory(
    ...
    // 1. 创建 Transcript
    let mut transcript =
        Transcript::new(&team_id, &request.user_id, request.role, &request.content);
    ...
    if let serde_json::Value::Object(ref mut map) = metadata {
        ...
        map.insert(
            ...
            serde_json::Value::String(transcript.created_at.to_rfc3339()),
        );
    }
    if let Err(e) = memory_state
        .search
...
pub async fn search_memory(
    ...
    match memory_state
        .search
        .search(&search_request, query_embedding.as_deref())
    {
