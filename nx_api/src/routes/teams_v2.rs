//! Team routes v2 — CLI-first execution
//!
//! Same REST API as teams.rs, but execute_team_task always uses the CLI path
//! (agent_team_service.execute_team_task) instead of PTY-first dispatch.
//! PTY terminal panel still works — it's just no longer the default execution path.
...
/// Execute a task across a team — CLI-first (v2)
///
/// Always uses agent_team_service.execute_team_task() (CLI path) which returns
/// clean stdout text. PTY terminal panel still works independently for viewing
/// execution processes, but is not used for task dispatch.
pub async fn execute_team_task(
    ...
    tracing::info!(
        "[RouteV2] execute_team_task CLI-first, team_id: {}",
        team_id
    );
    ...
    tokio::spawn(async move {
        ...
        match result {
            Ok(response) => {
                ...
                tokio::spawn(async move {
                    if let Err(e) =
                        store_raw_transcript(&memory_state, &team_id_for_memory, "user", &um).await
                    {
                        tracing::warn!("[MemoryV2] Failed to store user transcript: {}", e);
                    }
                    if !assistant_reply.is_empty() {
                        ...
                        {
                            tracing::warn!(
                                "[MemoryV2] Failed to store assistant transcript: {}",
                                e
                            );
                        }
                        ...
                        {
                            tracing::warn!(
                                "[MemoryV2] Structured storage failed, falling back: {}",
                                e
                            );
